...
import com.yupi.springbootinit.config.QQEmailConfig;
...
public class SendMessageOperation {
    ...
    public void sendMessage(String targetEmail, RedisTemplate<String,String> redisTemplate)  {
        ...
        try {
            ...
            // 设置邮件主题
            mail.setSubject("咸余羊 智能BI");

            String code = RandomUtil.getFourBitRandom();
            //设置数据的5分钟有效期限
            redisTemplate.opsForValue().set(LOGINCODEPRE+targetEmail,code,5, TimeUnit.MINUTES);
            // 设置邮件内容
            mail.setMsg("欢迎您使用咸余羊 智能BI。您的注册 or 登录 验证码为："+code+",验证码5分钟内有效!!!"+"[咸余羊 智能BI]");
            // 设置邮件发送时间
            mail.setSentDate(new Date());
            // 发送邮件
            mail.send();
        } catch (EmailException e) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR,"邮箱生成错误！");
        }
    }
}